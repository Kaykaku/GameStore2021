...
public class LoginController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        pgbLoading.setStyle("-fx-accent: #00FF00");
        ...
        Matcher matcher = regex.matcher(txt_Email_Register.getText().trim());
        txt_Email_Register.focusedProperty().addListener((o, oldVal, newVal) -> {
            if (txt_Email_Register.getText().isEmpty()) {
                ...
                if (matcher.find()) {
                    ...
                    lbl_Message_Register.setText("Email format is wrong!");
                    txt_Email_Register.getStyleClass().add("txt_Incorrect");
                }
            }
        });
    ...
    private void LoginAction(ActionEvent event) {
        ...
        Account account = dao.selectByID(usename);
    ...
    private void SendOTPAction(ActionEvent event) {
        AccountDAO dao = new AccountDAO();
        Account account = dao.selectByID(txt_Username_ChangePass.getText());
        if (!txt_Username_ChangePass.getText().isEmpty()) {
